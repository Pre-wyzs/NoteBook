...
## 1寻找EL UI样式并改变它（from 样式）

![image-20220614224310578](Typora_images/Echart教程/image-20220614224310578.png)

- 检查子dom

![image-20220614224521161](Typora_images/Echart教程/image-20220614224521161.png)

==我原来以为添加伪类后只能把样式作用在自己身上，现在发现原来可以作用在其他的元素上的，通过类选择器！！！草了！现在找到了变量，然后再.scss里面改就行了==

![image-20220614224827826](Typora_images/Echart教程/image-20220614224827826.png)

==绝了，居然能直接在样式文件中操作css3的变量！==



## 2解构的妙用（from js基础）

![image-20220614231905137](Typora_images/Echart教程/image-20220614231905137.png)

- **<font color='red'>后端返回给我们的是一个对象对吧，然后又得从data中拿出poi，这样写法就很JB繁琐，点来点去的容易一下子看迷糊了，现在推荐一种写法=》解构</font>**

![image-20220614232112097](Typora_images/Echart教程/image-20220614232112097.png)

- **这样子写妈的一下子简洁太多了，**![img](Typora_images/Echart教程/861DAAD2.png)

- **<font color='purple'>数据库返回给我们的对象往往很复杂，所以就产生了一个想法，就是把数库的对象中的一个个属性都存起来，随时取用就行了，这个想法就诞生了解构的思想，然后一个就是浅拷贝对象，这样在把数据库对象 映射 成我们自己需要的对象的时候就会方便很多，并且可以一次性取到后端所有数据的内容，一字不落，就很叼！</font>（灵活操作后端数据的想法）**

![image-20220614235250907](Typora_images/Echart教程/image-20220614235250907.png)





## 3vue/vite项目中img src（from 图片处理）

- **<font color='red'>请使用import导入本地图片，然后再写到src里面就可以了，其他写法显得很繁杂而且多余。。。</font>**



## 4确定一个变量是使用 const 或者是 let （from js基础-》const 与 let）

- **<font color='red'>其实很简单的，关键在于 一个 变量肯定是在一个函数中定义的啊对吧，所以如果在一个函数中变量会改变，那就用let，否则就用const一开始就定义好值确保它在函数执行完后不会改变就行了。</font>**
